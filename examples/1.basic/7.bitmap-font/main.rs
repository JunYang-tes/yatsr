...
impl BDF {
  fn draw_text(&self, img: &mut PixImage, x: u32, y: u32, letter_space: u32, text: &str) {
    ...
    for line in text.split('\n') {
      ...
      for ch in line.chars() {
        if let Some(a) = self.glyphs.get(&ch) {
          crate::draw_bitmap_glypha(img, &a, x, y);
...
fn draw_bitmap_glypha(img: &mut PixImage, a: &Glyph, x: u32, y: u32) {
  ...
  for r in 0..a.height() {
    ...
    for byte in a.bitmap[r * a.cols as usize..(r + 1) * a.cols as usize].iter() {
      for i in 0..8 {
        if (byte >> (7 - i)) & 1 == 1 {
          print!("1");
          img.set_rgb(x as u32, y, Vec3::new(1., 0., 0.))
        } else {
          print!(" ");
        }
        x = x + 1;
...
fn main() {
  let mut img = PixImage::new(400, 200);
  let bdf = bdf_parser::parse(include_str!("./Chroma48Medium-8.bdf"));
  bdf.draw_text(&mut img, 10, 80, 4, "Hello,World");
  let bdf = bdf_parser::parse(include_str!("./CalLite24.bdf"));
  bdf.draw_text(&mut img, 10, 100, 4, "Hello,World");
  let bdf = bdf_parser::parse(include_str!("./CalBlk36.bdf"));
  bdf.draw_text(&mut img, 10, 150, 4, "Hello,World");
  let bdf = bdf_parser::parse(include_str!("./iv18x16u.bdf"));
  bdf.draw_text(&mut img, 10, 180, 4, "Hello,World");
